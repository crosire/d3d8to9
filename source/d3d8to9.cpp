...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateImageSurface(UINT Width, UINT Height, D3DFORMAT Format, Direct3DSurface8 **ppSurface)
{
	...
	*ppSurface = nullptr;
	...
	if (FAILED(hr))
	{
		LOG << "> 'IDirect3DDevice8::CreateImageSurface' failed!" << std::endl;
...
#define D3DVSD_END() 0xFFFFFFFF
	...
	if (pFunction != nullptr)
	{
		...
		if (FAILED(hr))
		{
			LOG << "> Failed to disassemble shader!" << std::endl;
		...
		if (FAILED(hr))
		{
			...
			{
				LOG << "> Failed to reassemble shader!" << std::endl;
			}
	...
	if (SUCCEEDED(hr))
	{
		...
		{
			LOG << "> 'IDirect3DDevice9::CreateVertexDeclaration' failed!" << std::endl;
	...
	{
		LOG << "> 'IDirect3DDevice9::CreateVertexShader' failed!" << std::endl;
	}
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::GetVertexShaderFunction(DWORD Handle, void *pData, DWORD *pSizeOfData)
{
	...
	LOG << "> Returning translated shader bytecode." << std::endl;
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreatePixelShader(CONST DWORD *pFunction, DWORD *pHandle)
{
	...
	*pHandle = 0;
	...
	if (FAILED(hr))
	{
		LOG << "> Failed to disassemble shader with!" << std::endl;
	...
	if (FAILED(hr))
	{
		LOG << "> 'IDirect3DDevice9::CreatePixelShader' failed!" << std::endl;
	}
